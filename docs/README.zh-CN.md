...
**Context Space 是面向生产环境的上下文工程基础设施，帮助开发者快速接入 MCP ，将 AI Agent 轻松连接到外部的数据和服务。**

当AI行业仍专注于提示词工程时，我们相信上下文工程才是大势所趋，希望Context Space为上下文工程的基础设置搭建贡献一份力量。
...
正如 Andrej Karpathy 最近[提到](https://x.com/karpathy/status/1937902205765607626)，上下文工程正在成为AI系统的基础，使其能够在复杂的真实世界中稳定运行。

上下文工程在[提示词工程](https://blog.langchain.com/context-engineering-for-agents/)的基础上进一步扩展，对AI模型推理过程中依赖的所有信息进行系统性设计与管理。

**核心组成**：
- 系统指令：指导模型行为的规则和示例
- 动态记忆：对话历史、持久性知识与上下文
- 信息检索：来自文档、API 或数据库的实时数据
- 可用工具：模型可以调用的功能（如搜索、发送邮件等）
- 用户状态：包括偏好设置、当前会话信息与个性化上下文
